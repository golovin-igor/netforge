...
namespace NetForge.Simulation.Topology.Common
{
    /// <summary>
    ...
    {
        private Dictionary<string, INetworkDevice> devices = new Dictionary<string, INetworkDevice>();
        private Dictionary<string, INetworkDevice> devicesByDeviceId = new Dictionary<string, INetworkDevice>();
        private Dictionary<string, PhysicalConnection> physicalConnections = new Dictionary<string, PhysicalConnection>();
        private readonly INetworkEventBus _eventBus = new NetworkEventBus();

        public INetworkEventBus EventBus => _eventBus;

        /// <summary>
        /// Add a device to the network
        /// </summary>
        public async Task AddDeviceAsync(INetworkDevice device)
        {
            if (device == null)
                return;
            if (!devices.ContainsKey(device.Name))
            {
                ...
                await _eventBus.PublishAsync(new DeviceChangedEventArgs(device.DeviceName, device, DeviceChangeType.Added));
            }
        }
        ...
        /// Get device by name
        /// </summary>
        public INetworkDevice GetDevice(string name)
        {
        ...
        public async Task RemoveDeviceAsync(string deviceName)
        {
            if (devices.TryGetValue(deviceName, out var device))
            {
                ...
                await _eventBus.PublishAsync(new DeviceChangedEventArgs(device.DeviceName, device, DeviceChangeType.Removed));
            }
        }
        ...
        /// </summary>
        public void UpdateDeviceIdMapping(INetworkDevice device, string? oldDeviceId = null)
        {
        ...
        /// </summary>
        public INetworkDevice? GetDeviceById(string deviceId)
        {
            if (string.IsNullOrEmpty(deviceId))
                return null;
        ...
        /// </summary>
        public INetworkDevice FindDeviceByIp(string ip)
        {
        ...
        /// </summary>
        public IEnumerable<INetworkDevice> GetAllDevices()
        {
        ...
        /// </summary>
        public List<(INetworkDevice device, string interfaceName)> GetConnectedDevices(string deviceName, string interfaceName)
        {
            var result = new List<(INetworkDevice device, string interfaceName)>();
