...
namespace NetForge.Simulation.Common.CLI.Factories
{
    ...
    {
        private static readonly ConcurrentDictionary<string, Func<INetworkDevice, IVendorContext>> _vendorContextFactories = new();
        private static readonly object _lockObject = new object();

        /// <summary>
        /// Register a vendor context factory
        /// </summary>
        public static void RegisterVendorContext<T>(string vendorName, Func<INetworkDevice, T> factory)
            where T : IVendorContext
        {
            var key = vendorName.ToLowerInvariant();
            var wrapper = new Func<INetworkDevice, IVendorContext>(device => factory(device));
        ...
        /// </summary>
        public static IVendorContext? GetVendorContext(INetworkDevice device)
        {
