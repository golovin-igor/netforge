    ...
    public class FortinetVendorCapabilities(INetworkDevice device) : IVendorCapabilities
    {
        ...
        public string GetRunningConfiguration()
        {
            ...
            foreach (var iface in interfaces.Values.OrderBy(i => i.Name))
            {
                ...
                config += $"    edit \"{iface.Name}\"\n";
                if (!string.IsNullOrEmpty(iface.IpAddress))
                    config += $"        set ip {iface.IpAddress} {iface.SubnetMask}\n";
                if (iface.IsShutdown)
                    config += "        set status down\n";
                config += "    next\n";
                config += "end\n";
            }
        ...
        {
            var fortinetModes = new[] { "user", "privileged", "config", "interface" };
            return fortinetModes.Contains(mode.ToLower());
        }
        ...
        public string GetVendorErrorMessage(string errorType, string? context = null)
        {
            ...
            {
                "invalid_command" => "Command fail. Invalid command.",
                "incomplete_command" => "Command fail. Incomplete command.",
                "invalid_parameter" => "Command fail. Invalid parameter.",
                "invalid_mode" => "Command fail. Command not available in current mode.",
                "permission_denied" => "Command fail. Permission denied.",
                "syntax_error" => "Command fail. Syntax error.",
                _ => $"Command fail. {errorType}"
            };
        }
        ...
        {
            var supportedFeatures = new[] { "firewall", "interface_configuration", "ip_routing", "vpn" };
