...
namespace NetSim.Simulation.CliHandlers
{
    ...
    /// </summary>
    public sealed class CliContext(NetworkDevice device, string[] commandParts, string fullCommand)
    {
        /// <summary>
        /// The device executing the command
        /// </summary>
        public NetworkDevice Device { get; } = device ?? throw new ArgumentNullException(nameof(device));
